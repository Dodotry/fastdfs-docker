...
if [ "$1" = "monitor" ] ; then
  if [ -n "$TRACKER_SERVER" ] ; then
    sed -i "s|tracker_server=.*$|tracker_server=${TRACKER_SERVER}:22122|g" /etc/fdfs/client.conf
  fi
  fdfs_monitor /etc/fdfs/client.conf
  exit 0
elif [ "$1" = "storage" ] ; then
  FASTDFS_MODE="storage"
  sed -i "s|store_path0.*$|store_path0=/home/fdfs|g" /etc/fdfs/mod_fastdfs.conf
  sed -i "s|url_have_group_name=.*$|url_have_group_name=true|g" /etc/fdfs/mod_fastdfs.conf
  cd /usr/local/nginx/sbin/ && ./nginx && cd ~
else
...
sed -i "s|tracker_server=.*$|tracker_server=${TRACKER_SERVER}:22122|g" /etc/fdfs/storage.conf
sed -i "s|tracker_server=.*$|tracker_server=${TRACKER_SERVER}:22122|g" /etc/fdfs/client.conf
